...
export default withEnglishFallback({
  ...
  pluginPlatform: {
    ...
    allowUnsignedSessionWarning: "在 DBX 重启前允许安装未签名插件包。此时安装的任何 .dbxp 或 URL 都会跳过签名校验，并能以你的用户权限运行原生代码。",
    ...
    workbenchNavigationBlocked: "插件页面试图离开其沙箱文档，已被阻止。仅在信任该插件时再重新加载。",
    reloadWorkbench: "重新加载插件页面",
  },
  auth: {
    ...
    setupToken: "初始化令牌",
    setupTokenHint: "请从 DBX 服务端日志中复制一次性初始化令牌（Docker 可执行 docker logs <容器名>）。",
    setupTokenInvalid: "初始化令牌无效，请从 DBX 服务端日志中重新复制。",
    hostNotAllowed: "不允许使用该主机名访问，请在服务端的 DBX_ALLOWED_HOSTS 中添加它。",
    passwordManagedByEnv: "密码由环境变量 DBX_PASSWORD 设置。请修改 DBX_PASSWORD 并重启 DBX Web。",
  },
  toolbar: {
  ...
  connection: {
    ...
    savedSecretPlaceholder: "已保存 — 留空则保持不变",
    savedSecretsTitle: "已保存的密钥",
    savedSecretsHint: "已保存的密钥不会显示。字段留空将保留已保存的值，输入新值将替换，也可以在此清除。",
    savedSecretClear: "清除已保存",
    savedSecretUndo: "撤销",
    savedSecretCleared: "保存时移除",
    savedSecretConnectionString: "连接串",
    savedSecretUrlParams: "敏感 URL 参数",
    savedSecretUrlParamsHint: "已隐藏的敏感参数值（如 password=）留空时会保留原值。",
    savedSecretInitScript: "已保存的初始化脚本已隐藏 — 留空保持不变，输入新脚本将替换。",
    savedSecretPluginField: "插件密钥（{key}）",
  ...
  tableVGroup: {
    ...
    groupDeleted: "分组已删除",
    groupsMenu: "表分组",
    databaseGroupsMenu: "数据库分组",
    moveDatabaseToGroup: "移动到数据库分组",
    newDatabaseGroup: "新建数据库分组",
    newDatabaseRuleGroup: "按正则新建数据库分组",
    newDatabaseRuleGroupMenu: "按正则新建分组...",
    newRuleGroup: "按正则新建分组",
    newRuleGroupMenu: "按正则新建分组...",
    newRuleSubgroup: "按正则新建子分组...",
    addRule: "添加正则规则...",
    editRule: "编辑正则规则",
    saveRule: "保存规则",
    clearRule: "移除规则",
    ...
    patternInvalid: "正则表达式无效：{message}",
    patternTooLong: "表达式过长（最多 500 个字符）。",
    patternMatchCount: "已加载的 {total} 项中匹配 {count} 项",
    patternMoreMatches: "……另有 {count} 项",
    patternHint: "手动放入分组的项目保持不变；其余项目会进入第一个匹配的规则分组，之后新增的数据库和表也会自动归组。",
  },
  connectionGroup: {
    createGroup: "新建分组",
    group: "分组",
  ...
  configExport: {
    ...
    passphraseTooShortMin: "密码短语至少需要 {min} 个字符",
    passphraseMinLengthHint: "至少使用 {min} 个字符。",
  ...
  ai: {
    ...
    deepLinkConfirmTitle: "AI 配置链接",
    deepLinkConfirm: "有链接请求添加 AI 配置“{name}”。提示词、查询上下文以及你填写的 API Key 将发送到：\n\n{endpoint}\n\n仅在你信任该地址时继续。",
  ...
  esDiscover: {
    ...
    placeholders: {
      dql: "使用 DQL 搜索，例如 level:ERROR and service.name:api*",
      lucene: "使用 Lucene 搜索，例如 level:ERROR AND message:timeout",
      ppl: "PPL，例如 where level = 'ERROR' {'|'} stats count() by service.name（自动添加 source=）",
      sql: "SQL，例如 SELECT level, COUNT(*) FROM `logs-*` GROUP BY level",
    },
    refresh: "刷新",
    commonlyUsed: "常用",
    quickRanges: {
    ...
    apply: "应用",
    rangeHint: "本地时间或日期表达式（now-1h）",
    invalidRange: "时间范围无效",
    dqlSyntaxError: "DQL 语法错误",
    addFilter: "添加筛选",
    removeFilter: "移除筛选",
    includeResults: "包含结果",
    excludeResults: "排除结果",
    ...
    enableAll: "全部启用",
    removeAll: "全部移除",
    filtersNotApplied: "筛选仅适用于 DQL 和 Lucene",
    not: "非",
    selectField: "选择字段",
    searchFields: "搜索字段",
    value: "值",
    ...
    addColumn: "添加",
    removeColumn: "移除",
    toggleColumn: "在表格中显示/隐藏列",
    topValues: "前 {count} 个值",
    noValuesInSample: "已加载的文档中没有值",
    emptyString: "（空字符串）",
    existsIn: "在 {exists} / {total} 条已加载记录中存在",
    filterFor: "筛选此值",
    filterOut: "排除此值",
    hits: "条命中",
    rows: "{count} 行",
    ...
    copied: "已复制",
    loadMore: "加载更多",
    sampleLimit: "仅显示前 {count} 条文档，请细化搜索条件查看其他文档。",
    showRenderedRows: "已显示 {shown} / {total} 条已加载行，显示全部",
    loading: "正在搜索…",
    enterIndexPattern: "输入索引模式（例如 logs-*）开始浏览文档。",
    requestFailed: "搜索失败",
    noResults: "没有符合搜索条件的结果",
    noResultsHint: "请尝试其他查询或移除部分筛选。",
    noResultsHintTime: "请尝试扩大时间范围、修改查询或移除筛选。",
    noRows: "查询未返回任何行",
    runHint: "按 Enter 或点击刷新运行查询。",
    pplUnsupported: "PPL 需要 OpenSearch SQL/PPL 插件；当前集群为 Elasticsearch，请改用 SQL。",
  },
  solrAdmin: {
    title: "Solr 管理",
    refresh: "刷新",
  ...
  dataCompare: {
    ...
    rowCountsTruncated: "源表 {source} 行，目标表 {target} 行；已达到全量比较行数上限",
    truncatedPartial: "该表超过全量比较行数上限，仅比较了部分行，生成的同步 SQL 不完整。大表请使用数据传输功能。",
    truncatedMissingTarget: "目标表不存在且源表超过全量比较行数上限：仅生成了 CREATE TABLE，未包含行数据。请使用数据传输功能复制数据。",
    truncatedBanner: "{count} 张表超过全量比较行数上限，比较结果和同步 SQL 不完整。大表请使用数据传输功能。",
  ...
  settings: {
    ...
    syncHttpsRequired: "请使用 https:// 地址。仅本机地址（localhost、127.0.0.0/8、::1）允许使用 http://。",
    syncPassphraseTooShort: "同步密码至少需要 {min} 个字符。",
    syncPassphraseTooShortExisting: "新的同步密码至少需要 {min} 个字符。较短的已有密码仍可用于下载，但不会被保存，也不能用于上传。",
    syncImportConfirmTitle: "确认云同步导入",
    syncImportConfirmDescription: "尚未应用任何内容。导入快照前请检查以下变更。",
    syncImportUnauthenticated: "此快照未经认证，任何对同步存储有写权限的人都可能修改过它。",
    syncImportEndpointChangesHint: "以下连接在本地保存了凭据，但快照会修改它们的连接地址：",
    syncImportKeepCredentials: "导入并保留已保存的凭据",
    syncImportClearCredentials: "导入并清除已变更连接的凭据",
    syncImportCancelled: "已取消导入，未做任何更改。",
    syncImportDroppedCredentials: "已清除 {count} 个已变更连接的保存凭据。",
    mcpPolicyUnconfiguredReadOnlyHint: "尚未保存 MCP 策略，因此 MCP 访问默认为只读。选择模式或范围后将保存策略。",
    ...
    mcpToolKafkaCluster: "Kafka 集群概览",
    mcpToolKafkaListTopics: "列出 Kafka 主题",
    mcpToolKafkaDescribeTopic: "查看 Kafka 主题详情",
    mcpToolKafkaTopicConfig: "Kafka 主题配置",
    mcpToolKafkaListConsumerGroups: "列出 Kafka 消费组",
    mcpToolKafkaConsumerGroup: "Kafka 消费组详情",
    mcpToolKafkaLag: "Kafka 消费延迟",
    mcpToolKafkaThroughput: "Kafka 吞吐与停滞状态",
    mcpToolOpensearchListIndices: "列出 OpenSearch 索引模式和索引",
    mcpToolOpensearchResolvePattern: "展开 OpenSearch 索引模式",
    mcpToolOpensearchFields: "列出 OpenSearch 字段",
    mcpToolOpensearchSearch: "搜索 OpenSearch（DQL/Lucene，时间窗口）",
    mcpToolOpensearchSql: "执行 OpenSearch SQL",
    mcpToolOpensearchPpl: "执行 OpenSearch PPL",
  ...
  esConsole: {
    ...
    copyFailed: "复制失败：{message}",
    requestEditorLabel: "控制台请求",
    responseLabel: "响应",
    responsesCount: "{count} 个响应",
    copyResponse: "复制响应",
    took: "{ms} 毫秒",
    running: "正在运行…",
    noResponse: "运行请求后在此查看响应。",
    noRequest: "请将光标放在某个请求上（例如 GET _search）再运行。",
    requestFailed: "请求失败",
    formatInvalidJson: "无法自动缩进：请求体不是有效的 JSON。",
    formatUnsupported: "请求体包含注释或三引号字符串，无法自动缩进。",
    confirmMessage: "此请求可能会不可逆地修改或删除数据。确定要发送吗？",
    confirmAction: "发送请求",
    readOnlyTitle: "只读连接",
    readOnlyError: "此连接为只读，集群未被修改。请为该连接解锁写入，或使用可写连接发送写请求。",
    splitterLabel: "调整请求与响应面板大小",
    history: "历史",
    historyTitle: "最近的请求（{count}）",
    historyEmpty: "此连接还没有运行过请求。",
    historyClear: "清空历史",
    historyInsert: "插入到编辑器",
    historyTruncated: "请求体过大，仅保留请求行",
    help: "帮助",
    helpTitle: "键盘快捷键",
    shortcutRun: "运行光标所在的请求，或所有选中的请求",
    shortcutIndent: "自动缩进当前请求",
    shortcutComment: "切换行注释",
    shortcutComplete: "显示补全建议",
    shortcutUndo: "撤销",
    helpSyntax: '每个请求单独一行写成 METHOD path，后面可跟 JSON 请求体（_bulk 和 _msearch 使用 NDJSON）。使用 # 或 // 写注释，使用 """ 编写多行字符串。',
  },
  codeSnapshot: {
    title: "代码截图",
    close: "关闭",
