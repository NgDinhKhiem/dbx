...
use axum::Json;
use dbx_core::persistence::connection_export::decrypt_connection_export_blocking;
...
/// Decrypts a version 1 (PBKDF2) or version 2 (Argon2id) config export on the
/// blocking pool; key derivation is deliberately expensive.
pub async fn decrypt_config(Json(body): Json<DecryptConfigRequest>) -> Result<Json<String>, AppError> {
    decrypt_connection_export_blocking(body.payload, body.passphrase).await.map(Json).map_err(AppError::from)
}

#[cfg(test)]
mod tests {
    use super::sql_file_upload_max_bytes_from_mb;
    use dbx_core::persistence::connection_export::decrypt_connection_export;
    ...
    fn exported_browser_payload() -> serde_json::Value {
        serde_json::json!({
    ...
    fn decrypts_browser_exported_config_payload() {
        let plaintext = decrypt_connection_export(&exported_browser_payload(), "passphrase").unwrap();
    ...
    fn rejects_wrong_config_passphrase() {
        let error = decrypt_connection_export(&exported_browser_payload(), "wrong").unwrap_err();
