...
| `setup_token_required` | Setup must include the one-time token printed in the server log (false for a browser on the server itself) |
...
When no password is configured, DBX prints a one-time setup token in its log (`docker logs <container>` for Docker). Clients that are not on the server itself must send it as `setup_token`; otherwise setup returns `403` with `SETUP_TOKEN_INVALID`. Prefer setting `DBX_PASSWORD` for server deployments.
...
| `DBX_HOST` | Listen address (default `127.0.0.1`; the Docker image sets `0.0.0.0`) |
...
```json
{
  "env": {
    ...
    "DBX_WEB_PASSWORD": "${DBX_WEB_PASSWORD}"
  }
}
```
