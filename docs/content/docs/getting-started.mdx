...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Docker">
    ...
    The Docker image supports the following startup variables. Set one with `-e NAME=value`:

    | Variable | Default | Purpose |
    | --- | --- | --- |
    | `DBX_PASSWORD` | Not set | Web login password. Set a strong value; the compose files refuse to start without it |
    ...
    | `DBX_HOST` | `0.0.0.0` in the image | Listen address. The Web server defaults to `127.0.0.1`; the image listens on all container interfaces so the published port works |
    | `DBX_PUBLIC_BASE_PATH` | `/` | Deploy below a subpath, such as `/dbx` |
    | `DBX_RUN_AS_ROOT` | Not set | Set to `1` to keep running as root (see below) |

    Then open `http://localhost:4224`.

    The examples publish the port on `127.0.0.1` only. To open DBX from other machines, put a TLS reverse proxy in front of it, or publish on all interfaces (`-p 4224:4224`) on a trusted network.

    If no password is configured, DBX prints a one-time setup token in the container log. A browser that is not on the server itself, which includes the host browser reaching a published container port, must enter it when setting the first password:

    ```bash
    docker logs dbx 2>&1 | grep -i -A1 "setup token"
    ```

    The container runs DBX as the unprivileged user `dbx` (uid/gid `10001`). It starts as root only to hand `/app/data` and `/app/backups` to that user, which also upgrades volumes created by older root-based images, and then drops privileges. If a mounted file, such as a `0600` root-owned secret, cannot be made readable by uid `10001`, set `DBX_RUN_AS_ROOT=1`. When you start the container with `--user`, make sure the data directory is writable by that user. The image has a health check against `/api/auth/check`.

    `deploy/docker-compose.yml` builds the current checkout. To deploy a published image, use `deploy/docker-compose.release.yml`:

    ```bash
    export DBX_PASSWORD='change-this-password'   # or put it in a .env file
    ...
    services:
      dbx:
        ...
        pull_policy: always
        ports:
          # Loopback only by default; set DBX_BIND=0.0.0.0 to publish on all interfaces.
          - "${DBX_BIND:-127.0.0.1}:4224:4224"
        environment:
          # Required: compose refuses to start when DBX_PASSWORD is unset.
