...
| `setup_token_required` | 初始化时必须携带服务器日志中打印的一次性令牌（服务器本机上的浏览器为 false） |
...
未配置密码时，DBX 会在日志中打印一次性初始化令牌（Docker 可用 `docker logs <容器名>` 查看）。不在服务器本机上的客户端必须通过 `setup_token` 提交该令牌，否则返回 `403`（`SETUP_TOKEN_INVALID`）。服务器部署建议直接设置 `DBX_PASSWORD`。
...
| `DBX_HOST` | 监听地址，默认 `127.0.0.1`；Docker 镜像设置为 `0.0.0.0` |
